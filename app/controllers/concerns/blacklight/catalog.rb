...
module Blacklight::Catalog
  ...
  included do
    ...
    def show
      ...
      if (sidecar.data["configured_fields"].key? :items) && (sidecar.data["configured_fields"][:items].length > 0)
		  sidecar.data["configured_fields"][:items].each do |item_id|
			 resp, doc = fetch item_id
			 @docs.push(doc)
			 @resps.push(resp)
		  end
	  elsif (sidecar.data["configured_fields"].key? "items") && (sidecar.data["configured_fields"]["items"].length > 0)
	  	  sidecar.data["configured_fields"]["items"].each do |item_id|
			 resp, doc = fetch item_id
			 @docs.push(doc)
			 @resps.push(resp)
    ...
    def facet
      id = params[:id].gsub("*",".")
      @facet = blacklight_config.facet_fields[id]
      Rails.logger.warn("In catalog#facet:"+@facet.inspect)
